...
static bool WideToCharMap(const wchar * Src, char * Dest, size_t DestSize,
                          bool & Success);
static void CharToWideMap(const char * Src, wchar * Dest, size_t DestSize,
                          bool & Success);
...
#ifdef _WIN_ALL
    if (WideCharToMultiByte(CP_ACP, 0, Src, -1, Dest, (int)DestSize, NULL,
                            NULL) == 0) {
        RetCode = false;
    }
...
bool WideToCharMap(const wchar * Src, char * Dest, size_t DestSize,
                   bool & Success)
{
...
// We use it for extended ASCII names in Unix.
void CharToWideMap(const char * Src, wchar * Dest, size_t DestSize,
                   bool & Success)
{
    ...
    while (*Src != 0 && --dsize >= 0) {
        ...
        if (c < 0x80) {
            ...
            *(Dest++) = (0x80 | (c & 0x3f));
            ...
            if (c >= 0xd800 && c <= 0xdbff && *Src >= 0xdc00 &&
                    *Src <= 0xdfff) { // Surrogate pair.
    ...
    for (; *Src != 0; Src++)
        if (*Src < 0x80) {
            ...
            Size += 2;
                   ...
                   0x10000) { //(uint) to avoid Clang/win "always true" warning for 16-bit wchar_t.
            if (Src[0] >= 0xd800 && Src[0] <= 0xdbff && Src[1] >= 0xdc00 &&
                ...
                Size += 3;
                   ...
                   0x200000) { //(uint) to avoid Clang/win "always true" warning for 16-bit wchar_t.
            Size += 4;
        }
...
bool UtfToWide(const char * Src, wchar * Dest, size_t DestSize)
{
    ...
    while (*Src != 0) {
        ...
        if (c < 0x80) {
            ...
            d = ((c & 7) << 18) | ((Src[0] & 0x3f) << 12) | ((Src[1] & 0x3f) << 6) |
                (Src[2] & 0x3f);
            Src += 3;
        } else {
            Success = false;
