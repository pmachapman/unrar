...
void ArcCharToWide(const char * Src, wchar * Dest, size_t DestSize,
                   ACTW_ENCODING Encoding)
{
...
{
#ifdef _WIN_ALL
    return CompareStringA(LOCALE_USER_DEFAULT, NORM_IGNORECASE | SORT_STRINGSORT,
                          s1, -1, s2, -1) - 2;
#else
...
{
    for (int I = (int)wcslen(Str) - 1; I >= 0 && (Str[I] == '\r' ||
            Str[I] == '\n' || Str[I] == ' ' || Str[I] == '\t'); I--) {
        Str[I] = 0;
    }
    return Str;
...
void BinToHex(const byte * Bin, size_t BinSize, char * HexA, wchar * HexW,
              size_t HexSize)
{
