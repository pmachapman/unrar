...
public:
    ...
    virtual bool Open(const wchar * Data, uint Mode = FMF_READ);
    ...
    static void SetCloseFileTimeByName(const wchar * Name, RarTime * ftm,
                                       RarTime * fta);
    void GetOpenFileTime(RarTime * ft);
    virtual bool IsOpened()
    {
        return FileSize != 0;
    }; // 'virtual' for MultiFile class.
    int64 FileLength();
...
#ifdef _WIN_ALL
    void RemoveSequentialFlag()
    {
        NoSequentialRead = true;
    }
#endif
#ifdef _UNIX
    int GetFD()
